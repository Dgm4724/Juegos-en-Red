...
Se debe instalar Visual Studio Code en la máquina con las extensiones de SpringBoot y Java instaladas. Acto seguido se abre en el editor la carpeta raíz del proyecto (la que contenga todos los archivos, en este caso se llama F_$_GRUPO_01). Se hace clic derecho sobre el archivo DemoApplication, que se encuentra en la ruta src/main/java/com/example/demo. A continuación se levantará el servidor, y cuando termine este proceso se deberá abrir el navegador e introducir la siguiente URL: http://localhost:8080. Ya está disponible la aplicación para jugar.
